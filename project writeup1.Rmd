...
```
The model fits perfectly, which is suspicious. Checking variable importance for potential leak of the target variable.
```{r, warning=F}
...
confusionMatrix(predTree, as.factor(testing$classe))$table
confusionMatrix(predTree, as.factor(testing$classe))$overall[1] # test set accuracy
confusionMatrix(predlda, as.factor(testing$classe))$table
confusionMatrix(predlda, as.factor(testing$classe))$overall[1] # test set accuracy
```

All two models preforms as expected, the deviation from the cross validation accuracy is low and I do not see a reason to change resampling method or adding repetitons.
LDA seems superior to rpart tree model, in terms of accuracy. Choosing to predict on pmltest with the lda model.
Checking if there is anything to gain from increasing the number of boosting iterations.

```{r}
#plot(fitlda)
print(fitlda$bestTune)
```
...
```{r}
preds <- predict(fitlda, pmltest)
data.frame(cases, preds)
cat("Predictions: ", paste(predict(fitlda, pmltest)))
```
